...
class GraphConvolution(torch.nn.Module):
    ...
    def __init__(self, in_features, out_features, mesh='body', bias=True):
        ...
        if mesh=='body':
            adj_indices = torch.load('data/base_data/graph/smpl_431_adjmat_indices.pt')
            adj_mat_value = torch.load('data/base_data/graph/smpl_431_adjmat_values.pt')
            adj_mat_size = torch.load('data/base_data/graph/smpl_431_adjmat_size.pt')

        self.adjmat = torch.sparse_coo_tensor(adj_indices, adj_mat_value, size=adj_mat_size).cuda()
