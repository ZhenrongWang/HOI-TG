...
class SMPLH(object):
    ...
    def getdownsample(self, x, n1=0, n2=None):
        """Downsample mesh."""
        if n2 is None:
            n2 = self.num_downsampling
        if x.ndimension() < 3:
            x1 = spmm(self._D[0].to(x.device), x)
            x2 = spmm(self._D[1].to(x.device), x1)
        elif x.ndimension() == 3:
            out1 = []
            out2 = []
            for i in range(x.shape[0]):
                y = x[i]
                y1 = spmm(self._D[0].to(x.device), y)
                y2 = spmm(self._D[1].to(x.device), y1)
                out1.append(y1)
                out2.append(y2)
            x1 = torch.stack(out1, dim=0)
            x2 = torch.stack(out2, dim=0)
        return x1,x2
    ...
    def getupsample(self, x, n1=2, n2=0):
        """Upsample mesh."""
        if x.ndimension() < 3:
            for i in reversed(range(n2, n1)):
                x = spmm(self._U[i].to(x.device), x)
            ...
            for i in range(x.shape[0]):
                y = x[i]
                y1 = spmm(self._U[1].to(x.device), y)
                y2 = spmm(self._U[0].to(x.device), y1)
                out1.append(y1)
                out2.append(y2)
            x1 = torch.stack(out1, dim=0)
            x2 = torch.stack(out2, dim=0)
...
class InteractObject(object):
    def __init__(self, data):
        ...
        self.graph = np.array(data['graph']).astype(np.float32)
